...
from main import sort_uniq_names, top_three_popular_name, longest_shortest_courses, add_folder_yandex_disk, \
    check_folder_yandex_disk
...
class TestSort_uniq_names(TestCase):
    ...
    def test_slice_sort_uniq_names(self):
        ...
        expected = ['Адилет', 'Александр', 'Алексей', 'Алена', 'Анатолий', 'Анна', 'Антон', 'Вадим', 'Владимир',
                    'Денис', 'Евгений', 'Елена', 'Иван', 'Илья', 'Кирилл', 'Константин', 'Максим', 'Никита', 'Олег',
                    'Павел', 'Ринат', 'Роман', 'Сергей', 'Тимур', 'Филипп', 'Эдгар', 'Юрий']
        self.assertEqual(real_res, expected)
...
class TestLongest_shortest_courses(TestCase):
    ...
    def test_same_data_longest_shortest_courses(self):
        ...
        real_res = longest_shortest_courses(self.mentors, self.courses, self.new_durations)
        expected = ([], ['Python-разработчик с нуля', 'Java-разработчик с нуля', 'Fullstack-разработчик на Python',
                         'Frontend-разработчик с нуля'])
        self.assertEqual(real_res, expected)
        self.assertIsInstance(real_res, tuple)
...
class TestPytest_sort_uniq_names:
    ...
    @pytest.mark.parametrize(
        'ment, expected', [
            ([['Евгений Шмаргунов'], ['Анна Юшина'], ['Михаил Ларченко'], ['Адилет Асканжоев']],
             ['Адилет', 'Анна', 'Евгений', 'Михаил']),
            ([['Александр Беспоясов'], ['Александр Фитискин'], ['Александр Шлейко'], ['Александр Иванов']],
             ['Александр'])
        ]
    )
